...
const Payments: React.FC = () => {
  ...
  const getStatusBadge = (status: string, t: any) => {
    switch (status) {
      case 'completed':
        return <span className="inline-flex px-2 text-xs font-semibold leading-5 text-green-800 bg-green-100 rounded-full">{t('completed')}</span>;
      case 'pending':
        return <span className="inline-flex px-2 text-xs font-semibold leading-5 text-yellow-800 bg-yellow-100 rounded-full">{t('pending')}</span>;
      case 'failed':
        return <span className="inline-flex px-2 text-xs font-semibold leading-5 text-red-800 bg-red-100 rounded-full">{t('failed')}</span>;
      default:
        return <span className="inline-flex px-2 text-xs font-semibold leading-5 text-gray-800 bg-gray-100 rounded-full">{t(status)}</span>;
    }
  };
    ...
    <Layout isLoggedIn={isLoggedIn}>
      <div className="container px-4 py-8 mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">{t('viewManagePayments')}</p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="p-6 text-center bg-white rounded-lg shadow">
            <p className="text-xl text-gray-600">{t('noPaymentRecords')}</p>
          </div>
        ) : (
                  ...
                  <div className="px-4 py-4 sm:px-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        <p className="ml-2 text-sm font-medium text-blue-600 truncate">
                          {t(payment.payment_method)} - ৳{payment.amount}
                        </p>
                      </div>
                    ...
                    <div className="mt-2 sm:flex sm:justify-between">
                      ...
                      <div className="flex items-center mt-2 text-sm text-gray-500 sm:mt-0">
                        ...
                        <p>
                          {new Date(payment.payment_date).toLocaleDateString(i18n.language === 'bn' ? 'bn-BD' : 'en-US', {
                            year: 'numeric',
                            month: 'short',
                            day: 'numeric',
                          })}
        ...
        <div className="p-6 mt-8 bg-white rounded-lg shadow">
          <h2 className="mb-4 text-xl font-semibold text-gray-800">{t('makeNewPayment')}</h2>
          <p className="mb-4 text-gray-600">
            {t('makeNewPaymentDesc')}
          </p>
